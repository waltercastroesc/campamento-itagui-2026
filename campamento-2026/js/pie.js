...
function pintarPie(contenedor, contactos) {
  ...
  // El logo va primero (izquierda) y WhatsApp despues (derecha), en ese orden.
  fila.append(construirMarcas(), construirEnlaces(contactos));

  envoltorio.append(fila);
...
/** El logo real de WhatsApp, en linea y coloreado con currentColor (vino). */
...
function construirEnlaces(contactos) {
  ...
  if (contactos.whatsapp && !contactos.whatsapp.startsWith("PENDIENTE")) {
    ...
    whatsapp.append(construirEtiquetaConIcono(crearIconoWhatsapp(), "Grupo de WhatsApp", "del Campamento"));
